 ...
 const checkttDataPath = __dirname + '/../modules/commands/checktuongtac/';
  setInterval(async() => {
    ...
    if (day != day_now) {
      ...
      api.sendMessage({body:checkttBody,attachment: fs.createReadStream(__dirname + `/../modules/commands/noprefix/ttngay.gif`)},  checkttFile.replace('.json', ''), (err) => err ? console.log(err) : '');
...
url: (await global.nodemodule["axios"]('https://caochungdat.me/docs/other/videoanime')).data.url,
method: "GET",
responseType: "stream"
})).data
