module.exports.config = {
    ...
    switch (console.log(i), t.type) {
        case "reply":
            ...
            if ("ban" == i[0] || "Ban" == i[0]) {
                (h = i.slice(2).join(" ")) || (h = "Không có lý do nào được đưa ra"), e.sendMessage("◆━━◆THÔNG BÁO◆━━◆\n\nĐã nhận lệnh BAN nhóm từ admin\nVới lý do: " + h, d);
            ...
            if ("banout" == i[0] || "Banout" == i[0]) {
                (h = i.slice(2).join(" ")) || (h = "Không có lý do nào được đưa ra"), e.sendMessage("◆━━◆THÔNG BÁO◆━━◆\n\nĐã nhận lệnh BANOUT nhóm từ admin\nVới lý do: " + h, d);
                let s = (g = await e.getThreadInfo(d)).threadName;
                const r = (await a.getData(d)).data || {};
                r.banOut = d, await a.setData(d, {
                    data: r
                }), e.sendMessage("[" + d + "] :\n" + s + "\nĐã banout thành công!\nvới lý do: " + h, n.threadID, () => e.unsendMessage(t.messageID));
            }
            if ("out" == i[0] || "Out" == i[0]) {
                (h = i.slice(2).join(" ")) || (h = "Không có lý do nào được đưa ra");
                let a = (g = await e.getThreadInfo(d)).threadName;
                e.sendMessage("◆━━◆THÔNG BÁO◆━━◆\n\nĐã nhận lệnh out nhóm từ admin\nVới lý do: " + h, d, () => e.removeUserFromGroup("" + e.getCurrentUserID(), d)), e.sendMessage("Đã out thread có id: " + d + "\n" + a + "\nVới lý do: " + h, n.threadID, () => e.unsendMessage(t.messageID));
            }
            if ("Sendbox" == i[0] || "sendbox" == i[0]) {
                var h;
                if (!(h = i.slice(2).join(" "))) return e.sendMessage("Nội dung thông báo không được để trống!", n.threadID, () => e.unsendMessage(t.messageID));
                let a = (g = await e.getThreadInfo(d)).threadName;
                e.sendMessage("◆━━◆THÔNG BÁO◆━━◆\n\nThông Báo Từ ADMINBOT Gửi Đến Nhóm Bạn!\n◆━━━━━━━━━━━━━◆\n\n" + h, d), e.sendMessage("Đã gửi thông báo tới thread có id:\n[" + d + "]\n" + a, n.threadID, () => e.unsendMessage(t.messageID));
            }
            if ("Info" == i[0] || "info" == i[0]) {
                ...
                var b = 0 == y ? "tắt" : 1 == y ? "bật" : "Kh",
                    T = 0 == y ? "❎" : 1 == y ? "✅" : "⭕";
                return e.sendMessage("Tên box: " + i + "\nID Box: " + h + "\n" + T + " Phê duyệt: " + b + "\nEmoji: " + o + "\n-Thông tin:\nTổng " + a + " thành viên\n👨‍🦰Nam: " + f + " thành viên \n👩‍🦰Nữ: " + p + " thành viên\n\n🕵️‍♂️Với " + s + " quản trị viên gồm:\n" + D + "\nTổng số tin nhắn: " + r + " tin.", n.threadID, () => e.unsendMessage(t.messageID));
            }
            break;
        case "replyuser":
    ...
    if ("thread" == a[0]) {
        ...
        var s = function(e, n) {
            ...
            r = [];
        if (1 == a.length || !a) return e.sendMessage("» Tổng nhóm đã tham gia : " + m.length + "\n» Số Trang : " + s.length + "\n\n» " + l + "list thread (số trang)!", n.threadID, n.messageID);
        ...
        e.sendMessage(I + "» Trang(" + a[1] + "/" + s.length + ")\n\nReply [out, ban, banout, info, sendbox]\nSố thứ tự để [out, ban, banout, info, sendbox] thread đó!!", n.threadID, (e, a) => {
    ...
    if ("user" == a[0]) {
        ...
        if (1 == a.length || !a) return e.sendMessage("» Tổng user sử dụng BOT : " + global.data.allCurrenciesID.length + "\n» Số Trang : " + g.length + "\n\n» " + s + "list user (số trang)!", n.threadID, n.messageID);
        let r = "",
            o = 1;
        var l = [];
        for (var u of g[a[1] - 1]) {
            ...
            r += o++ + ". Tên: " + a + "\nTID: " + u + "\n😘 Giới tính: " + (2 == m ? "🙋‍♂️ Nam" : 1 == m ? "🙋‍♀️ Nữ" : "🤦‍♀️ Trần Đức Bo") + "\n\n", l.push(u);
        }
        e.sendMessage(r + "» Trang(" + a[1] + "/" + g.length + ")\n\nReply [info]\nSố thứ tự để [info] user đó!!", n.threadID, (e, a) => {
            global.client.handleReply.push({
                name: this.config.name,
                author: n.senderID,
                messageID: a.messageID,
