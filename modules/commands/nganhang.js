...
module.exports.run = async function ({ api, event, args, Currencies, Users }) {
    ...
    const checkBank = (await axios.get(`https://tuandz.herokuapp.com/bank/check?ID=${senderID}`)).data   
    const { createReadStream } = require(`fs-extra`);
    switch(args[0]) {
        case 'register':
        case '-r':
        case 'r': {
            const res = (await axios.get(`https://tuandz.herokuapp.com/bank/register?senderID=${senderID}&name=${encodeURI((await Users.getData(senderID)).name)}`)).data
        ...
        case "-f": {
            ...
            let { data } = (await axios.get(`https://tuandz.herokuapp.com/bank/find?type=${args[1].toUpperCase()}&${args[1].toUpperCase()}=${args[2]}`))
      ...
      case '-c': {
        ...
        const res = (await axios.get(`https://tuandz.herokuapp.com/bank/find?type=ID&ID=${a}`)).data  
        ...
        case 'rút': {
         ...
         case '-t':{
            if(checkBank.status == false) return api.sendMessage('[ 𝐁𝐀𝐍𝐊 ] - 𝐁𝐚̣𝐧 𝐜𝐡𝐮̛𝐚 𝐜𝐨́ 𝐭𝐚̀𝐢 𝐤𝐡𝐨𝐚̉𝐧 𝐭𝐫𝐞̂𝐧 𝐧𝐠𝐚̂𝐧 𝐡𝐚̀𝐧𝐠', threadID, messageID);
            const res = (await axios.get(`https://tuandz.herokuapp.com/bank/top`)).data  
        ...
        case 'nạp': {
            ...
            const res = (await axios.get(`https://tuandz.herokuapp.com/bank/send?senderID=${senderID}&money=${args[1]}`)).data  
        ...
        case 'pw': {
            ...
            switch(type) {
                case 'get': {
                    const res = (await axios.get(`https://tuandz.herokuapp.com/bank/password?bka=${type}&dka=${senderID}`)).data 
...
module.exports.handleReply = async function ({ api, event, handleReply, Currencies }) {
    ...
    switch(handleReply.type) {
        case 'paySTK': {
            const res = (await axios.get(`https://tuandz.herokuapp.com/bank/pay?type=STK&senderID=${senderID}&STK=${handleReply.STK}&money=${handleReply.money}&password=${body}`)).data 
            if(res.status == false) return api.sendMessage(res.message, threadID, messageID);
            api.sendMessage(`${res.message.noti}\n${res.message.data.message}`, threadID, messageID);
            return api.sendMessage(`${res.message.noti}\n\n${res.message.data.message}`, handleReply.threadID);
        }
        case 'payID': {
            const res = (await axios.get(`https://tuandz.herokuapp.com/bank/pay?type=ID&senderID=${senderID}&userID=${handleReply.ID}&money=${handleReply.money}&password=${body}`)).data 
            if(res.status == false) return api.sendMessage(res.message, threadID, messageID);
            api.sendMessage(`${res.message.noti} ${res.message.data.message}`, threadID, messageID);
            return api.sendMessage(`${res.message.noti}\n\n${res.message.data.message}`, handleReply.threadID);
        }
        case 'getMoney': {
            const res = (await axios.get(`https://tuandz.herokuapp.com/bank/get?ID=${senderID}&money=${handleReply.money}&password=${body}`)).data  
            if(res.status == false) return api.sendMessage(res.message, threadID, messageID);
        ...
        case 'newPassword': {
            const res = (await axios.get(`https://tuandz.herokuapp.com/bank/password?bka=recovery&dka=${senderID}&fka=${body}`)).data  
            if(res.status == false) return api.sendMessage(res.message, threadID, messageID);
